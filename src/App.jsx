...
function App() {
  ...
  const background = [espacio1, espacio2, espacio3,  espacio4, espacio5, espacio6, espacio7]
  
  const [imageVisible, setImageVisible] = useState(Math.floor(Math.random() * (background.length -1)))
  ...
  const changeImage = () =>{
    ...
    if (newImageVisible > background.length -1)
      ...
      else if (newImageVisible == imageVisible) setImageVisible(Math.floor(Math.random() * (background.length -1)));
      else setImageVisible(newImageVisible)
  }
  
  const [dataVisible, setDataVisible] = useState(Math.floor(Math.random() * (data.length -1)));
  ...
  const changeData = () => {
    ...
    if (newDataVisible > data.length -1)
      ...
      else if (newDataVisible == dataVisible) setDataVisible(Math.floor(Math.random() * (data.length -1)));
